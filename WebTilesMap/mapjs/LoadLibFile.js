...
function handleLocation(points, pointCoords){
    ...
    if(pointCoords){
        ...
        for (var i = 1; i <= Ext.Cat.MapConfig.MaxZoomLevel; i++) {
        ...
        $('sliderbar_' + map.model.getId()).parentNode.style.top = ((Ext.Cat.MapConfig.MaxZoomLevel - level) * 12 + 6) + "px";
...
function locate(e, x, y, info){
    ...
    marker.setIcon(new Icon(13, 24, Ext.Cat.MapConfig.ImageBaseDir + "marker_small.png"));
    marker.setInfo(info);
    marker.setToMap(mapDiv, map.model);
    
    map.model.setViewCenterCoord(marker.getCoord());
    map.model.setZoom(new Zoom(Ext.Cat.MapConfig.MaxZoomLevel));
