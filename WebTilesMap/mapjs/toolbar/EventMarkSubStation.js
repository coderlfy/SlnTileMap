MarkSubStationEvent = function (tool, baseStationStore) {
    var baseStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: '../../../handler/SecurityMonitor/RealMonitor/DeviceVirtrualUI.ashx?action=SubstationSerial',
            method: 'post'
        }),
    ...
    var SubStationNo = new Ext.form.ComboBox({
        hiddenName: 'SubStationNo',
        mode: 'local',
        triggerAction: 'all',
        store: baseStore,
        editable: false,
    ...
    var selectSubStationPanel = new Ext.form.FormPanel({
        ...
        items:
            [
                SubStationNo,
                { fieldLabel: '监测分站IP', id: 'IpAdress', xtype: 'textfield', disabled: true },
                { fieldLabel: '所在位置', id: 'Palce', xtype: 'textfield', disabled: true },
                { fieldLabel: '监测分站标定坐标', id: 'Pos', xtype: 'textfield', disabled: true }
            ]
    });
            ...
            [
                {
                    ...
                    handler: function () {
                        Ext.Ajax.request({
                            method: 'post',
                            url: '../../../handler/SecurityMonitor/RealMonitor/DeviceVirtrualUI.ashx?action=SubPOSUpdate',
                            params: { SubStationID: SubStationNo.getValue(), Pos: Ext.get("Pos").getValue() },
