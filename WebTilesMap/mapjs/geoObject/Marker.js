...
Ext.define('iCatMap.Marker', {
    ...
    constructor: function () {
        ...
        me.prefixId = me.prefixId ? 'Over_' + me.prefixId : 'Over_Marker';
        if (me.markerId) {
            me.id = me.prefixId + "_" + me.markerId;
            me.infoDivId = me.prefixId + "_Info_" + me.markerId;
        }
        else {
            me.id = iCatMap.Util.createUniqueID(me.prefixId + "_");
            me.infoDivId = iCatMap.Util.createUniqueID(me.prefixId + "_Info");
        }
    },
    getCoord: function () {
        return this.coord;
    ...
    setToMap: function (mapDiv, model, overLayDiv, fn) {
        this.mapDiv = mapDiv;
        this.model = model;
        this.sPoint = iCatMap.Util.getScreenPixel(this.coord, model.getZoom());
        var infoDiv = null;
        var inforDeltaX = 0, inforDeltaY = 0;
        var deltaX = this.sPoint.x;
        var deltaY = this.sPoint.y;
        var markerDiv = iCatMap.Util.createDiv(this.id, deltaX, deltaY, this.icon.width, this.icon.height, null, 'absolute');
        markerDiv.style.zIndex = 10;
        markerDiv.style.cursor = 'pointer';
        markerDiv.style.background = "url(" + this.icon.src + ")";
        ...
        if (this.info) {
            inforDeltaX = this.sPoint.x + (this.icon.width + this.offsetLeft);
            inforDeltaY = this.sPoint.y - (this.icon.height - this.offsetTop);
            infoDiv = iCatMap.Util.createDiv(this.infoDivId, inforDeltaX, inforDeltaY, null, 0, null, 'absolute');
        }
        //缩放地图时，重绘地图上图标。
        if (overLayDiv) {
            this.infoDiv = overLayDiv;
