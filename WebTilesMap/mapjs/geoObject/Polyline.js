...
Ext.extend(Ext.Cat.AjaxMap.Polyline, Ext.Cat.AjaxMap.AbstractOverLayer, {
    ...
    setToMap: function(mapDiv, model, overLayerDiv){
        ...
        var pixel = Util.getScreenPixel(new Ext.Cat.AjaxMap.Coordinate(this.getExtent().getMinX(), this.getExtent().getMaxY()), curZoom); //经纬度转屏幕
        
        var lines = new Array();
        lines.push('<v:PolyLine filled="false" Points="');
        for(var i=0; i<this.points.length; i++){
            var sPoint = Util.getScreenPixel(new Ext.Cat.AjaxMap.Coordinate(this.points[i].getCoord().x, this.points[i].getCoord().y), curZoom);
