//Map Overview
Ext.Cat.AjaxMap.OvMap = function (container) {
    this.container = container;
    this.drawOvContainer(container);
};
Ext.extend(Ext.Cat.AjaxMap.OvMap, Ext.Cat.AjaxMap.AbstractControl, {
    paint: function (model) {
        ...
        this.ovDiv.style.width = (curZoom.getTileCols() * Ext.Cat.MapConfig.TileSize) + "px";
        this.ovDiv.style.height = (curZoom.getTileRows() * Ext.Cat.MapConfig.TileSize) + "px";
    ...
    drawOvContainer: function (container) {
        var containerWidth = Util.getValueOfNoPX(container.dom.style.width);
        var containerHeight = container.dom.clientHeight;

        var width = Util.getValueOfNoPX(container.dom.style.width) / 5;
        var height = Util.getValueOfNoPX(container.dom.style.height) / 5;
        var x = width / 2 - width / 4 / 2;
        var y = height / 2 - height / 4 / 2;

        var left = containerWidth - width + 1 - 20;
        var top = containerHeight - height + 1 - 54;
        ...
        this.ovMapDiv.style.backgroundImage = 'url(' + Ext.Cat.MapConfig.ImageBaseDir + 'iaspec_bottom.png)';
        ...
        this.panDiv = Util.createDiv(Util.createUniqueID('Ov_Pan_'), null, null, null, null, Ext.Cat.MapConfig.ImageBaseDir + '1.GIF', 'absolute');
    ...
    registerEvent: function (source, prefix, param) {
        ...
        if (params) {
            for (var i = 0; i < params.length; i++) {
                ...
                Ext.get(source).on(params[i], eval('this.' + prefix + params[i]), this);
            }
        }
    },
    ...
    reLoadTiles: function (model, deltaX, deltaY, isPlus) {
        ...
        var realDeltaX = deltaX * scale * curZoom.realMapBound.getWidth() / (curZoom.getTileCols() * Ext.Cat.MapConfig.TileSize);
        var realDeltaY = deltaY * scale * curZoom.realMapBound.getHeight() / (curZoom.getTileRows() * Ext.Cat.MapConfig.TileSize);
        if (isPlus) {
